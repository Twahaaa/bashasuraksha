...
const geocodeRegion = async (region) => {
  ...
  console.warn(
    `Region "${region}" is not a coordinate pair. Geocoding skipped.`
  );
  return null;
};
        ...
        <div className="p-4 overflow-y-auto space-y-3 custom-scrollbar">
          {audioSamples.map((sample) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div
                    ...
                    style={{
                      backgroundColor:
                        clusterColors[sample.clusterId] || "#94a3b8",
                    }}
                  />
              ...
              {sample.transcript ? (
                <p className="text-sm text-neutral-200 mb-3 italic">
                  &quot;{sample.transcript}&quot;
                </p>
              ) : (
...
export default function MapContainer() {
  ...
  const updateMarkers = React.useCallback(
    (mapInstance) => {
      ...
      geocodedData.forEach(({ region, samples, coordinates }) => {
        ...
        badge.style.cssText =
          "position:absolute;top:-8px;right:-8px;background:#FACC15;color:black;border-radius:50%;width:20px;height:20px;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:bold;font-family:monospace;border:2px solid black;pointer-events:none;";
  ...
  useEffect(() => {
    ...
    if (samplesData.length > 0 && Object.keys(clusterColors).length === 0)
      return;
    if (isLoading) return;
    ...
    if (geocodedData.length > 0) {
      center = [
        geocodedData[0].coordinates.lng,
        geocodedData[0].coordinates.lat,
      ];
      zoom = 5;
    }
    ...
    <>
      ...
      >
        ...
        <div className="w-full flex-1 border border-neutral-800 bg-neutral-900/50 backdrop-blur-sm p-3 mb-4 min-h-0 relative">
          ...
          <div
            ref={mapContainer}
            className="w-full h-full border border-neutral-700 overflow-hidden"
          />
        </div>
        ...
        >
          ...
          <span>
            MAP SYSTEM ONLINE • {samplesData.length} SAMPLES •{" "}
            {geocodedData.length} REGIONS
          </span>
        </motion.div>
      </motion.div>
